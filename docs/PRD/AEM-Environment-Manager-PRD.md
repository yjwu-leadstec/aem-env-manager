...
| AEM 许可证管理 | P1 | 管理 AEM 6.5 的 license.properties 文件 |
...
管理 AEM 6.5 (On-Premise) 的 license.properties 许可证文件。AEM as a Cloud Service 使用云端订阅模式，无需本地许可证管理。

#### 2.9.2 许可证配置项

| 配置项 | 必填 | 说明 |
|--------|------|------|
| 名称 | 是 | 许可证的显示名称（如"Client A 生产许可证"） |
| 许可证文件 | 是 | license.properties 文件 |
| 客户名称 | 自动 | 从许可证文件解析 |
| 产品版本 | 自动 | 从许可证文件解析 |
| 到期日期 | 自动 | 从许可证文件解析 |
| 备注 | 否 | 自定义备注信息 |
...
| license.properties | AEM 6.5 的许可证文件，包含客户信息和有效期 |
